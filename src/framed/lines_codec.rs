// Forked from `tokio-util`, but the internal `Decoder` and `Encoder` are modified with `Bytes`.
...
fn utf8(buf: &[u8]) -> io::Result<&str> {
    str::from_utf8(buf)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "Unable to codec input as UTF8"))
}
